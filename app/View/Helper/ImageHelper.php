...
class ImageHelper extends AppHelper{
    ...
    public function resizedUrl($file, $width, $height){
        ...
        $file       = WWW_ROOT.'img/upload/' . trim($file, '/');
        $output     = $pathinfo['dirname'] . '/' . $width . 'x' . $height .'_' . $pathinfo['filename'] . '.' . $pathinfo['extension'];
        $file_output = WWW_ROOT .'img/upload/' . $output;
        if (!file_exists($file_output)) {
            ...
            switch ( $info[2] ) {
              case IMAGETYPE_GIF:   imagegif($image_resized, $file_output, $this->quality);    break;
              case IMAGETYPE_JPEG:  imagejpeg($image_resized, $file_output, $this->quality);   break;
              case IMAGETYPE_PNG:   imagepng($image_resized, $file_output, 9);    break;
              default: return false;
            }
        }
